class Vertice: # classe para definir um vertice no grafo, que representa uma cidade
...
class Adjacente: # classe para define a relação entre um vértice (cidade) e uma cidade adjacente
    def __init__(self, vertice, custo):
        self.vertice = vertice
        self.custo = custo

class Grafo: # classe grafo para manter a lista de vértices (cidades):
    def __init__(self):
        self.vertices = []
...
contenda = Vertice('Contenda', 39)
 ...
 # Criação das adjacentes, usamos a classe adjacente para adicionar cidades conectadas e suas distâncias
...
grafo = Grafo() #construcao do grafo 
# os vértices são adicionados ao grafo
...
grafo.mostra_adjacentes() # método para exibir todas as cidades e suas conexões.
